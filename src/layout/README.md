...
| `DwellStateMachine` / `DwellPhase.h` / `DwellRegionSpace` / `InvalidGazeGrace.h` | Dwell timing. Unspecified cells use rapid vs standard from settings (`usesRapidDwell`). Shared blink/scan grace via `setDwellTiming`. `<Phase>` cells: first activation enters phase 0, later steps wrap last→first; leave after blink grace commits. Progress pauses during blink grace |

...
| `DwellStateMachine` / `DwellPhase.h` / `DwellRegionSpace` / `InvalidGazeGrace.h` | Dwell timing. Unspecified cells use daily-driver vs designer from settings (`usesDailyDriverDwell`). `<Phase>` cells arm/advance with each dwell step and fire on leave |

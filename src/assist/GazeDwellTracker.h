...
/// Shared progressive gaze dwell with stable/freeze/cancel radii.
...
public:
    ...
    bool sample(const QPointF& gaze, double dtSec)
    {
        ...
        const double dx = gaze.x() - m_commitPos.x();
        const double dy = gaze.y() - m_commitPos.y();
        const double dist = qSqrt(dx * dx + dy * dy);

        if (dist <= double(m_stableRadiusPx)) {
            m_commitPos.setX(m_commitPos.x() * (1.0 - m_commitTrackAlpha)
                             + gaze.x() * m_commitTrackAlpha);
            m_commitPos.setY(m_commitPos.y() * (1.0 - m_commitTrackAlpha)
                             + gaze.y() * m_commitTrackAlpha);
            m_progress = qBound(0.0, m_progress + dtSec * 1000.0 / double(m_dwellMs), 1.0);
        } else {
            // Drifted off the commit point. Bleed progress and walk commit toward
            // the live point so a new dwell can start as soon as the user settles.
            const double bleed =
            ...
            m_commitPos.setX(m_commitPos.x() * (1.0 - pull) + gaze.x() * pull);
            m_commitPos.setY(m_commitPos.y() * (1.0 - pull) + gaze.y() * pull);
            if (m_progress <= 0.001) {
                m_commitPos = gaze;
                m_progress = 0.0;
            }
